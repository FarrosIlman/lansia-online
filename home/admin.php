...
$sql = $koneksi->query("SELECT COUNT(id_pasien) as pend  from tb_pasien where status='Ada'");
while ($data = $sql->fetch_assoc()) {
	$pend = $data['pend'];
}

$sql = $koneksi->query("SELECT COUNT(id_penanggung) as kartu  from tb_penanggung");
while ($data = $sql->fetch_assoc()) {
	$penang = $data['kartu'];
}
...
$sql = $koneksi->query("SELECT COUNT(id_pasien) as laki  from tb_pasien where jekel='LK'");
while ($data = $sql->fetch_assoc()) {
	$laki = $data['laki'];
}

$sql = $koneksi->query("SELECT COUNT(id_pasien) as prem  from tb_pasien where jekel='PR'");
while ($data = $sql->fetch_assoc()) {
	$prem = $data['prem'];
}
